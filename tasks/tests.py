...
class TaskViewTest(TestCase):
    ...
    def test_filter_by_multiple_params(self):
        response = self.client.get(reverse('tasks:tasks') + '?completed=true&priority=HIGH')
        self.assertEqual(response.status_code, 200)
        self.assertIn(self.task1, response.context['object_list'])
        self.assertNotIn(self.task2, response.context['object_list'])
        self.assertNotIn(self.task3, response.context['object_list'])
...
class TaskToggleStatusViewTest(TestCase):
    def test_toggle_task_status(self):
        user = get_user_model().objects.create_user(username='testuser', password='password')
        self.client.login(username='testuser', password='password')
        task = Task.objects.create(name='Test Task', description='Test Description', is_completed=False)
        response = self.client.post(reverse('tasks:task-toggle-status', kwargs={'pk': task.pk}))
...
class TaskCreateViewTest(TestCase):
    def test_create_task(self):
        ...
        project = Project.objects.create(name='Project A', description='Project A Description', team=team)
...
class TaskDeleteViewTest(TestCase):
    def test_delete_task(self):
        user = get_user_model().objects.create_user(username='testuser', password='password')
        self.client.login(username='testuser', password='password')
        task = Task.objects.create(name='Test Task', description='Task Description', is_completed=False)
